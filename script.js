...
const filterMenuBySelectedTag = (clickedTag) => {
  ...
  let i = 0;
  subtags.forEach((subtags) => {
    if (subtags.classList.contains('menu__block_hidden')) i++;
  });
  if (subtags.length === i) {
...
window.addEventListener('scroll', function () {
  let height = document.querySelector('.html').scrollTop;
  if (height > 200) {
    fadeIn('#back2Top');
  } else {
    fadeOut('#back2Top');
...
window.addEventListener('resize', () => {
  if (window.innerWidth < 961) {
    const back2Top = document.querySelector('.link-contacts');
    back2Top.setAttribute('href', '#contacts');
  }
});
